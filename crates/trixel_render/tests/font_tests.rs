...
use trixel_render::glyphs::{self, GLYPH_WIDTH, GLYPH_HEIGHT, free_cell_count};
...
fn single_char_constraint_count_less_than_total() {
    ...
    let constraints = TrixelFont::string_to_constraints("A", 0, 0);
...
fn constraints_only_contain_state_0_and_2() {
    let constraints = TrixelFont::string_to_constraints("HELLO", 0, 0);
...
fn constraints_respect_start_offset() {
    let constraints = TrixelFont::string_to_constraints("A", 5, 10);
...
fn multi_char_gap_is_transparent() {
    ...
    let constraints = TrixelFont::string_to_constraints("AB", 0, 0);
...
fn lowercase_treated_as_uppercase() {
    let upper = TrixelFont::string_to_constraints("A", 0, 0);
    let lower = TrixelFont::string_to_constraints("a", 0, 0);
...
fn text_has_light_backing_plate_border() {
    ...
    let constraints = TrixelFont::string_to_constraints("A", 5, 5);
...
fn multi_char_backing_plate_spans_full_width() {
    ...
    let constraints = TrixelFont::string_to_constraints("AB", 2, 3);
