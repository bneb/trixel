use trixel_core::gf3::{self, GF3, TRITS_PER_SYMBOL};
use trixel_core::{RsEcc, ErrorCorrection, LENGTH_PREFIX_TRITS};
use trixel_solver::gauss_solver::{compute_transform_matrix, build_parity_check_matrix, grid_to_flat_coords};
use trixel_solver::{GaussSolver, MatrixSolver};
use trixel_solver::anchor::{self, is_in_anchor_region};
...
fn gauss_solver_produces_valid_codeword() {
    ...
    let codeword_trits_len = trixel_core::decode_length(&flat[..LENGTH_PREFIX_TRITS]);
...
fn gauss_solver_respects_font_constraints() {
    let n = 20;
    let constraints = TrixelFont::string_to_constraints("HI", 4, 4);
...
fn gauss_solver_with_font_still_decodes() {
    ...
    use trixel_core::{TernaryCodec, MockCodec};
